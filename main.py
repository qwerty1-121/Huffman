...
# Функция для сохранения словаря кодирования
def save_encoding_dict(encoding_dict, file_name):
    with open(file_name, 'wb') as file:
        pickle.dump(encoding_dict, file)


# Функция для загрузки словаря кодирования
def load_encoding_dict(file_name):
...
def main():
    while True:
        choice = int(input(
            "Для шифрования файла введите 1\nДля расшифрования файла введите 2\nДля выхода введите 0\nВыбор режима: "))
        if choice == 0:
            break
        elif choice == 1:
            data = read_text_file("original.txt")
            encoding_dict = build_encoding_dict(data)
            save_encoding_dict(encoding_dict, 'huffman_encoding_dict.pkl')
            encoded_data = encode_data(data, encoding_dict)
            write_binary_file("encoded.bin", encoded_data)
            print("Файл успешно зашифрован.")
        elif choice == 2:
            encoded_data = read_binary_file("encoded.bin")
            encoding_dict = load_encoding_dict('huffman_encoding_dict.pkl')
